...
export default function App() {
  ...
  return (
    <div className="app"> {/* Contenedor principal con clase CSS 'app' */}
      <Logo /> {/* Componente que muestra el encabezado con el nombre de la aplicación */}
      <Form /> {/* Componente que permite añadir nuevos artículos a la lista */}
      <PackingList /> {/* Componente que renderiza la lista de artículos */}
      <Stats /> {/* Componente que muestra estadísticas sobre los artículos empacados */}
    </div>
  );
}
...
function Form() {
  ...
  return (
    ...
    <form className="add-form" onSubmit={handleSubmit}>
      <h3>What do you need for your 😍 trip?</h3> {/* Pregunta al usuario qué necesita para su viaje */}
      ...
      <select>
        {Array.from({ length: 20 }, (_, i) => i + 1).map((num) => (
          <option value={num} key={num}> {/* Cada opción tiene un valor numérico */}
            {num} {/* El texto mostrado es el número correspondiente */}
          </option>
        ))}
      </select>
...
function PackingList() {
  return (
    <div className="list"> {/* Contenedor con clase CSS 'list' */}
      <ul> {/* Lista no ordenada que contendrá los artículos */}
        {/* Mapea cada artículo en initialItems a un componente Item */}
        {initialItems.map((item) => (
          <Item item={item} key={item.id} /> {/* Pasa el artículo como prop y usa su id como clave única */}
        ))}
      </ul>
    </div>
  );
...
function Item({ item }) {
  return (
    <li> {/* Elemento de lista para un artículo individual */}
...
function Stats() {
  return (
    <footer className="stats"> {/* Pie de página con clase CSS 'stats' */}
      <em> {/* Texto en cursiva que contendrá las estadísticas */}
        🎒 You have X items on your list, and you already packed X
        (X%) {/* Mensaje placeholder - se actualizará dinámicamente con números reales */}
      </em>
    </footer>
  );
}
